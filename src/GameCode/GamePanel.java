...
public class GamePanel extends JPanel implements Runnable, KeyListener, PlayerObserver  {
	boolean tre = true;
	...
	public void loadLevel(int level) throws IOException {
		...
		SoundManager.getInstance().loopSound("./Images/background.wav");
		...
		// Wall және Pool объектілерін фабрика арқылы жасау
		walls = new Wall[readInt()];
		pools = new Pool[readInt()];
		for(int i=0;i<walls.length;i++) walls[i] = GameElementFactory.createWall(readInt(), readInt(), readInt(), readInt());
		for(int i=0;i<pools.length;i++) pools[i] = GameElementFactory.createPool(readInt(), readInt(), readInt(), readInt());

		// Player объектілерін GameElementFactory арқылы жасау
		fireboy = GameElementFactory.createPlayer('w', 'a', 'd', readInt(), readInt(), Color.red, readInt(), readInt());
		watergirl = GameElementFactory.createPlayer((char)38, (char)37, (char)39, readInt(), readInt(), Color.blue, readInt(), readInt());

		fireboy.addObserver(this);
		watergirl.addObserver(this);
	...
	@Override
	public void onLevelComplete(Player player) {
		if(tre){
			System.out.println("Observer: " + player.colour + " есікке жетті!");
			tre = false;
		}

		if (fireboy.atDestination() && watergirl.atDestination()) {
			System.out.println("Fireboy және Watergirl есікке жетті!");
			gameFinished = true;
		}
	}
